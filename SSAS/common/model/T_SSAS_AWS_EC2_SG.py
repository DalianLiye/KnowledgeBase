from sqlalchemy import Column, TIMESTAMP, String
from sqlalchemy.orm import declarative_base
from sqlalchemy.ext.hybrid import hybrid_property

Base = declarative_base()


class T_SSAS_AWS_EC2_SG(Base):
    __tablename__ = 'SSAS_AWS_EC2_SG'

    _account_id = Column("account_id", String, primary_key=True)
    _vpc_id = Column("vpc_id", String, primary_key=True)
    _security_group_id = Column("security_group_id", String, primary_key=True)
    _security_group_name = Column("security_group_name", String)
    _security_group_description = Column("security_group_description", String)
    _create_time = Column("create_time", TIMESTAMP)

    @hybrid_property
    def account_id(self):
        return self._account_id

    @account_id.setter
    def account_id(self, value):
        self._account_id = value

    @hybrid_property
    def vpc_id(self):
        return self._vpc_id

    @vpc_id.setter
    def vpc_id(self, value):
        self._vpc_id = value

    @hybrid_property
    def security_group_id(self):
        return self._security_group_id

    @security_group_id.setter
    def security_group_id(self, value):
        self._security_group_id = value

    @hybrid_property
    def security_group_name(self):
        return self._security_group_name

    @security_group_name.setter
    def security_group_name(self, value):
        self._security_group_name = value

    @hybrid_property
    def security_group_description(self):
        return self._security_group_description

    @security_group_description.setter
    def security_group_description(self, value):
        self._security_group_description = value

    @hybrid_property
    def create_time(self):
        return self._create_time

    @create_time.setter
    def create_time(self, value):
        self._create_time = value

    def __repr__(self):
        return f"<MyTable(account_id='{self._account_id}', " \
               f"vpc_id='{self._vpc_id}', " \
               f"security_group_id='{self._security_group_id}', " \
               f"security_group_name='{self._security_group_name}', " \
               f"security_group_description='{self._security_group_description}', " \
               f"create_time={self._create_time})>"