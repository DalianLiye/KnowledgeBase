...
Model profile可以让程序根据模型能力（上下文长度、tool_calling、模态、最大 token 等）动态自适应，不用硬编码\
Model profile目前属于beta测试功能，配置的格式未来可能会变化\
Model profile要求的langchain版本： >= 1.1

LangChain初始化chat model时，会自带一个.profile属性\
该属性是一个字典，里面包含模型支持的所有特性和能力\
这个字典就是 Model profile
...
LangChain的model profile数据主要来自开源项目models.dev\
models.dev是一个专门维护模型能力信息的开源项目\
LangChain会在其数据基础上扩展额外字段，并与上游models.dev保持同步更新


Model profile的核心价值：让应用程序根据模型自身能力动态调整行为，无需写死代码
例如：
