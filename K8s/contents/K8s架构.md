...
一般情况下，Pod只运行于工作节点，但也可以运行于控制平面\
因此控制平面和工作节点可以是同一台机器（集群可以只有一个机器）

注：单节点集群就是一台机器同时承担控制平面和工作节点双重角色，多用于测试、学习环境

官方约定配置：至少1个控制平面 + 至少1个工作节点

![k8s集群架构图](./img/K8s架构_001.jpg)
<img src="./img/K8s架构_001.jpg" alt="k8s集群架构图" width="100">
...
![控制平面架构图01](./img/K8s架构_002.png)\ 
![控制平面架构图02](./img/K8s架构_003.svg)  
...
注：控制平面是K8s集群的核心组件，一般生产环境会将其部署在多个节点上，实现高可用


## 节点
K8s的节点架构图如下：\
![节点架构图](./img/K8s架构_003.svg)
...
- **container runtime(容器运行时环境)**\
  ...
  **注：**
  - Docker并非K8s唯一支持的容器运行时
  - 需提前在节点服务器安装对应运行时\
    例如选用Docker时，要先在节点装好Docker，再部署K8s集群
  - 所有容器运行时只有遵循K8s的CRI标准，才能和kubelet正常交互
  - CRI:K8s定义的容器运行时接标准口（Container Runtime Interface）\
    CRI-O:兼容CRI的容器运行时
...
- **管理层**\
  ...
  策略管理：包含RBAC权限、资源配额、Pod 安全策略、网络策略等
...
**注：**\
分层架构图的上下仅为分类排版，不代表调用链路\
核心层提供最底层的功能，是底座\
应用层实现了集群内的相关应用服务，通过直接调用核心层实现\
管理层实现了针对应用层的监控和管理功能，也通过直接调用核心层实现\
接口层用于为生态系统层的外部工具提供接口，生态系统层的应用（kubectl,dashboard,各种插件）通过接口层调用核心层实现功能

核心层是底层基础，应用层、管理层直接调用核心层，二者互不调用\
接口层面向外部，kubectl、Dashboard、各类插件等外部工具，通过接口层调用核心层，无需经过管理层